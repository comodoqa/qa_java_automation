...
public class CLICK implements IAction{
	...
	private void doClick() {
		...
		try {
			...
			result = Action.screen.click(fullImageUrl);
		} catch (FindFailed e) {
			failMessage = String.format("Element [%s] not found", image);
			isPassed = false;
		} catch( Exception e) {
			//
		}
		
		if(result == 0 && isPassed == true) {
			failMessage = String.format("Failed to click on the element [%s]", image);
			isPassed = false;
		}
	}
